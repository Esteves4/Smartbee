...
class PubSubClient:
    ...
    def connect(self, ide, user=None, password=None, willTopic=0, willQos=0, willRetain=0, willMessage=0):
        if not self.connected():
            ...
            if result == 1:
                ...
                length = self.writeString(ide, self.buffer, length)
                if willTopic:
    ...
    def readPacket(self, lengthLength):
        ...
        for dummy in range(start, length):
            (success, result) = self.readByte()
            if not success:
                return 0
            digit = result
    ...
    def subscribe(self, topic, qos=0):
        ...
        if self.connected():
            ...
            return self.write(self.MQTTSUBSCRIBE|self.MQTTQOS1, self.buffer, length-5)
